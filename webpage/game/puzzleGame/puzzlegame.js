...
function move(id) {
    ...
    if (target_d != 0) {
        ...
        document.getElementById("a" + id).style.left = d_posXY[target_d][0] + "px";
        document.getElementById("a" + id).style.top = d_posXY[target_d][1] + "px";
...
function start() {
    if (pause) {
        document.getElementById("start").innerHTML = "PAUSE"; //把按钮文字设置为暂停
...
function random_d() {
    ...
    if (d[i] != 0) {
        document.getElementById("a" + d[i]).style.left = d_posXY[to][0] + "px";
        document.getElementById("a" + d[i]).style.top = d_posXY[to][1] + "px";
    }
    //把当前的DIV位置设置为随机产生的DIV的位置
    if (d[to] != 0) {
        document.getElementById("a" + d[to]).style.left = d_posXY[i][0] + "px";
        document.getElementById("a" + d[to]).style.top = d_posXY[i][1] + "px";
    }
